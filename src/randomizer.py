...
class Randomizer():
    ...
    def validate_manifest(self) -> bool | None:
        """Validates files and key types from manifest.
        ...
        Returns:
            True if all is valid, else None.
        """
        ...
        if not isinstance(res_validation, list):
            self.logger.error(
                f"Incorrect data type for \"resources_validation\" \
                key: list expected, got {type(res_validation)}"
            )
            raise ManifestKeyError(
                "resources_validation", type(res_validation)
            )
        ...
        if isinstance(server_paths, (list, str)):
            ...
            self.logger.error(
                f"Incorrect data type for \"servers_paths\" key: \
                list or str expected, got {type(server_paths)}"
            )
            raise ManifestKeyError("server_paths", type(server_paths))
        ...
        if isinstance(triggers_info, (dict)):
            ...
            self.logger.error(
                f"Incorrect data type for \"triggers_to_change\" key: \
                list or dict expected, got {type(server_paths)}"
            )
            raise ManifestKeyError("triggers_to_change", type(server_paths))
